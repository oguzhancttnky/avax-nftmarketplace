...
function Listings({walletAddress}){
    const [listings, setListings] = useState([]);

    useEffect(() => {
        const fetchListings = async() =>{
            try{
                const response = await axios.get(`http://localhost:8080/dbNft/listings/${walletAddress}`);
    ...
    return(
        <div className="listings__container">
            <h1 className="listings__title">User Listings</h1>            
            <div className="listings__grid">
                {listings.map(listing => (
                    <div key={listing.ID}>
                        <NFTCard nft={listing}/>
                        {listing.Price && <p className="nft-card__price">Price: {listing.Price} AVAX
